...
const float rec2020_primaries[3][2] = {
    {0.708486, 0.293545},
    {0.190182, 0.775398},
    {0.129252, 0.047142}
};
...
float find_distance_to_edge(float cos_angle, float sin_angle)
{
    ...
    for (int i = 0; i < 3; i = i+1) {
        ...
        const float x3 = rec2020_primaries[i][0];
        const float y3 = rec2020_primaries[i][1];
        const float x4 = rec2020_primaries[next_i][0];
        const float y4 = rec2020_primaries[next_i][1];
...
float[2] rotate_and_scale_primary(float scaling, float rotation,
                                  int primary_index)
{
    // Generate a custom set of tone mapping primaries by scaling
    // and rotating the primaries of the given profile.
    const float dx = rec2020_primaries[primary_index][0] - D50_whitepoint[0];
    const float dy = rec2020_primaries[primary_index][1] - D50_whitepoint[1];
    const float angle = atan2(dy, dx) + rotation;
    const float cos_angle = cos(angle);
    const float sin_angle = sin(angle);
    const float distance_to_edge = find_distance_to_edge(cos_angle, sin_angle);
...
void calculate_adjusted_primaries(float inset[3], float rotation[3],
                                  ...
                                  output float rendering_to_base[3][3])
{
              ...
              float b_inset, float b_rotation)
{
    ...
    float rendering_to_base[3][3];
    calculate_adjusted_primaries(inset, rotation,
                                 base_to_rendering, rendering_to_base);
    ...
    if (custom_primaries) {
        rgb = mult_f3_f33(rgb, rendering_to_base);
    }
